    ...
    public void SpawnBorderForest(List<Vector3> edgePositions, GameObject[] treePrefabs, int layers = 2, float density = 0.85f, float offsetRange = 0.5f)
    {
        for (int inset = 0; inset < layers; inset++)
        {
            ...
            foreach (Vector3 pos in edgePositions)
            {
                ...
                {
                    Vector3 randomOffset = new Vector3(UnityEngine.Random.Range(-offsetRange, offsetRange), 0,UnityEngine.Random.Range(-offsetRange, offsetRange)); // adding a random offset so all the trees arent in stright jarring lines 

                    Vector3 spawnPos = pos + randomOffset;

                    GameObject treePrefab = treePrefabs[UnityEngine.Random.Range(0, treePrefabs.Length)]; // randomly choosing which of the tree prefabs will spawn 
                    Quaternion rot = Quaternion.Euler(0, UnityEngine.Random.Range(0f, 360f), 0);

                    GameObject.Instantiate(treePrefab, spawnPos, rot); // spawning them with the random prefabs spawns and rotations 
                }
            }
        }
    }
